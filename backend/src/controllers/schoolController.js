...
async function updateNeed(req, res, next) {
  try {
    const need = await schoolService.updateNeed(parseInt(req.params.id, 10), parseInt(req.params.needId, 10), req.body, adminContext(req));
...
module.exports = { listSchools, getSchool, createSchool, updateSchool, deleteSchool, addNeed, updateNeed, deleteNeed, getFilters };
